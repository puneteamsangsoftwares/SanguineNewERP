...
public class clsSalesOrderController {
	...
	public @ResponseBody Set<clsSubGroupMasterModel> getSubgroupNames(
	 ...
	 * @return
	 */
	private Set<clsSubGroupMasterModel> simulateSubgroupNameSearchResult(
			String sgName) {
	...
	public ModelAndView funOpenForm(Map<String, Object> model,
		...
		Map<String, String> settlementList = objSettlementService
				.funGetSettlementComboBox(clientCode);
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmSalesOrder_1", "command",
					new clsSalesOrderBean());
		} else if ("1".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmSalesOrder", "command",
					new clsSalesOrderBean());
		} else {
			return null;
		}
	...
	public ModelAndView funSaveSO(
			@ModelAttribute("command") @Valid clsSalesOrderBean objSOBean,
		...
		String loginglocCode = request.getSession()
				.getAttribute("locationCode").toString();
		...
		// }
		if (!result.hasErrors()) {
			clsSalesOrderHdModel SOModel = funPrepardHDModel(objSOBean,
					request, currRate);
			...
			if (saveFlg) {
				if (null != listSODtlModel) {
					...
					for (clsSalesOrderDtl obSODtl : listSODtlModel) {
						if (null != obSODtl.getStrProdCode()) {
							...
							if (obSODtl.getDblAcceptQty() != 0) {
								...
								funPrepardBOMData(SOModel, obSODtl, clientCode,
										userCode);
							}
							intindex++;

							if (null != objSOBean.getListSalesChar()) {
								objSalesOrderService.funDeleteSalesChar(
										strSOCode, obSODtl.getStrProdCode());
								List<clsSalesCharModel> listSaleCharModel = objSOBean
										.getListSalesChar();
								for (clsSalesCharModel objSaleChar : listSaleCharModel) {
									if (objSaleChar.getStrProdCode().equals(
											obSODtl.getStrProdCode())) {
										objSaleChar.setStrSOCode(strSOCode);
										// objSaleChar.setStrProdCode(obSODtl.getStrProdCode());
										objSalesOrderService
												.funAddUpdateSaleChar(objSaleChar);
									}
								}
							}
					...
					List<clsSalesOrderTaxDtlModel> listaxDtlModel = objSOBean
							.getListSalesOrderTaxDtl();
					if (null != listaxDtlModel) {
						objSalesOrderService.funDeleteSalesOrderTaxDtl(
								strSOCode, clientCode);
						for (clsSalesOrderTaxDtlModel obTaxDtl : listaxDtlModel) {
							if (null != obTaxDtl.getStrTaxCode()) {
								...
								obTaxDtl.setStrTaxableAmt(obTaxDtl
										.getStrTaxableAmt() * currRate);
								obTaxDtl.setStrTaxAmt(obTaxDtl.getStrTaxAmt()
										* currRate);
								objSalesOrderService
										.funAddUpdateSoTaxDtl(obTaxDtl);
							}
						}
					}
					...
					if (objSetup != null && objSetup.getStrSOKOTPrint() != null) {
						request.getSession().setAttribute("PrintSOKOT",
								objSetup.getStrSOKOTPrint());
					}

					request.getSession().setAttribute("success", true);
					request.getSession().setAttribute("successMessage",
							"SO Code : ".concat(strSOCode));
					request.getSession().setAttribute("rptSOCode", strSOCode);
					request.getSession().setAttribute("CalStock", "No");
					return new ModelAndView(
							"redirect:/frmSalesOrder.html?saddr=" + urlHits);

				}
			}
			return new ModelAndView("frmSalesOrder?saddr=" + urlHits,
					"command", new clsSalesOrderBean());
	...
	private clsSalesOrderHdModel funPrepardHDModel(clsSalesOrderBean objBean,
			HttpServletRequest req, double currRate) {
		long lastNo = 0;
		String clientCode = req.getSession().getAttribute("clientCode")
				.toString();
		String userCode = req.getSession().getAttribute("usercode").toString();
		...
		if (objBean.getStrSOCode().trim().length() == 0) {
			strDocNo = objGlobalFunctions.funGenerateDocumentCode(
					"frmSalesOrder", objBean.getDteSODate(), req);
			SOHDModel = new clsSalesOrderHdModel(new clsSalesOrderHdModel_ID(
					strDocNo, clientCode));
			...
			clsSalesOrderHdModel objSalesModel = objSalesOrderService
					.funGetSalesOrderHd(objBean.getStrSOCode(), clientCode);
			if (null == objSalesModel) {
				strDocNo = objGlobalFunctions.funGenerateDocumentCode(
						"frmSalesOrder", objBean.getDteSODate(), req);
				SOHDModel = new clsSalesOrderHdModel(
						new clsSalesOrderHdModel_ID(strDocNo, clientCode));
				SOHDModel.setIntId(lastNo);
				SOHDModel.setStrUserCreated(userCode);
				SOHDModel.setDteDateCreated(objGlobal
						.funGetCurrentDate("yyyy-MM-dd"));
		...
		SOHDModel.setStrCurrency(objGlobalFunctions.funIfNull(
				objBean.getStrCurrency(), "INR", objBean.getStrCurrency()));
	...
	public @ResponseBody clsSalesOrderBean funAssignFields(
			@RequestParam("soCode") String soCode, HttpServletRequest req) {
		String clientCode = req.getSession().getAttribute("clientCode")
				.toString();
		String userCode = req.getSession().getAttribute("usercode").toString();
		String startDate = req.getSession().getAttribute("startDate")
		...
		List<Object> objSales = objSalesOrderService.funGetSalesOrder(soCode,
				clientCode);
		...
		if (objSales != null) {
			...
			objBeanSale = funPrepardHdBean(objSalesOrderHdModel,
					objLocationMasterModel, objPartyMasterModel);
			...
			List<Object> objSalesDtlModelList = objSalesOrderService
					.funGetSalesOrderDtl(soCode, clientCode);
			for (int i = 0; i < objSalesDtlModelList.size(); i++) {
				...
				saleDtl.setDblAvgQty(funGetCustAvg(
						objBeanSale.getStrCustCode(), saleDtl.getStrProdCode(),
						objBeanSale.getDteSODate(), req));
				...
				if (!(objSetup == null)) {
					if (objSetup.getStrShowAvgQtyInSO().equals("Y")) {
						double avgQty = funGetCustAvg(
								objBeanSale.getStrCustCode(),
								saleDtl.getStrProdCode(),
								objBeanSale.getDteSODate(), req);
						saleDtl.setDblAvgQty(avgQty);
					}
					if (objSetup.getStrShowStockInSO().equals("Y")) {
						String sqlStk = " select a.dblClosingStk "
								+ " from tblcurrentstock a where a.strLocCode='"
								+ objBeanSale.getStrLocCode() + "' "
								+ " and a.strProdCode='"
								+ saleDtl.getStrProdCode() + "' "
								+ " and a.strClientCode='" + clientCode + "'  ";
						List<BigDecimal> listSTK = objGlobalFunctionsService
								.funGetDataList(sqlStk, "sql");
	...
	public @ResponseBody List funAssignDetialsData(
			@RequestParam("soCode") String soCode, HttpServletRequest req) {
		String clientCode = req.getSession().getAttribute("clientCode")
				.toString();
	...
	private clsSalesOrderBean funPrepardHdBean(
			clsSalesOrderHdModel objSalesOrderHdModel,
			clsLocationMasterModel objLocationMasterModel,
			clsPartyMasterModel objPartyMasterModel) {
		clsSalesOrderBean objBeanSale = new clsSalesOrderBean();
		...
		objBeanSale.setDblSubTotal(objSalesOrderHdModel.getDblSubTotal()
				/ currRate);
		objBeanSale
				.setDblDisAmt(objSalesOrderHdModel.getDblDisAmt() / currRate);
		...
		objBeanSale.setStrSettlementCode(objSalesOrderHdModel
				.getStrSettlementCode());
		objBeanSale.setStrMobileNoForSettlement(objSalesOrderHdModel
				.getStrMobileNo());
	...
	private void funReportSalesOrderReport(
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private void funCallReportSalesOrderReport(String SOCode, String type,
			HttpServletResponse resp, HttpServletRequest req) {
		try {
			...
			String reportName = servletContext
					.getRealPath("/WEB-INF/reports/rptSalesOrderSlip.jrxml");
			...
			String sqlHd = "select a.strSOCode, date(a.dteSODate), b.strPName ,b.strBAdd1,b.strBAdd2 ,b.strBCity,b.strBState,b.strBPin,b.strBCountry,a.strNarration,a.dblDisAmt,a.dblSubTotal,a.dblTotal,a.dblConversion "
					...
					+ SOCode
					+ "' "
					+ "	and a.strCustCode=b.strPCode and a.strClientCode='"
			...
			if (!list.isEmpty()) {
				...
				dblDisAmt = Double.parseDouble(arrObj[10].toString())
						/ currRate;
				dblSubTotal = Double.parseDouble(arrObj[11].toString())
						/ currRate;
				dblTotal = Double.parseDouble(arrObj[12].toString()) / currRate;

			}

			String sqlDtl = " select a.strSOCode,b.strProdName,b.strPartNo,a.dblQty,b.strUOM,a.strRemarks,a.dblUnitPrice/"
					+ currRate
					+ " as dblUnitPrice "
			...
			JRDesignDataset subDataset = (JRDesignDataset) datasetMap
					.get("dsSalesReportDtl");
			subDataset.setQuery(subQuery);

			String Sql3 = "select a.strTaxDesc,a.strTaxAmt as strTaxAmt from tblsalesordertaxdtl a where strSOCode='"
					+ strSOCode + "' and strClientCode='" + clientCode + "'";
			...
			String SqlTax = "select a.strTaxDesc,sum(a.strTaxAmt) as strTaxAmt from tblsalesordertaxdtl a where strSOCode='"
					+ strSOCode
					+ "' and strClientCode='"
					+ clientCode
					+ "' group by strSOCode ";
			...
			clsPropertyMaster objPropertyMaster = objPropertyMasterService
					.funGetProperty(propertyCode, clientCode);
			if (clientCode.equals("319.001")
					&& objPropertyMaster.getPropertyName().equalsIgnoreCase(
							"TARANG FOODS")) {
	...
	private void funDeliveryNoteReport(
			@ModelAttribute("command") clsReportBean objBean,
			HttpServletResponse resp, HttpServletRequest req) {
		...
		funSalesOrderListReport(custCode, type, fromDate, toDate,
				fromFulfillment, toFulfillment, reportType, resp, req);
	}

	@SuppressWarnings({ "unchecked", "rawtypes" })
	private void funSalesOrderListReport(String custCode, String type,
			String fromDate, String toDate, String fromFulfillment,
			String toFulfillment, String reportType, HttpServletResponse resp,
			HttpServletRequest req) {
		...
		try {
			...
			if (reportType.equals("Summary")) {
				reportName = servletContext
						.getRealPath("/WEB-INF/reports/rptSalesOrderListSummary.jrxml");
			} else {
				reportName = servletContext
						.getRealPath("/WEB-INF/reports/rptSalesOrderListDetail.jrxml");
			...
			String sqlHd = "select Date(a.dteSODate),Date(a.dteFulmtDate),b.strPName from tblsalesorderhd a, tblpartymaster b  "
					...
					+ "and a.dteSODate between '"
					+ fromDate
					+ "' and '"
					+ toDate
					+ "' and a.dteFulmtDate between '"
					+ fromFulfillment + "' and '" + toFulfillment + "' ";
			...
			if (reportType.equals("Summary")) {
				sqlDtl = "select Date(a.dteSODate),Date(a.dteFulmtDate),b.strPName,a.strSOCode,a.strCustCode, "
						...
						+ toFulfillment
						+ "' "
						+ " and b.strPropCode='" + propertyCode + "' ";
			} else {
				sqlDtl = "select a.strSOCode,a.strProdCode ,b.strProdName,a.dblQty,a.strRemarks "
						+ " "
						+ " from tblsalesorderdtl a,tblproductmaster b,tblsalesorderhd c , tblpartymaster d  "
						+ "  "
						+ " where c.strCustCode='"
						+ custCode
						+ "' and a.strSOCode=c.strSOCode  and  c.strClientCode='"
						+ clientCode
						+ "'  "
						+ " "
						+ " and c.strClientCode=b.strClientCode and a.strProdCode=b.strProdCode "
						+ " and c.dteSODate between '"
						+ fromDate
						+ "' and '"
						+ toDate
						+ "'"
						+ "  and  "
						+ " c.dteFulmtDate between '"
						+ fromFulfillment
						+ "' and '"
						+ toFulfillment
						+ "' "
						+ " and c.strClientCode=d.strClientCode and c.strCustCode=d.strPCode  and d.strPropCode='"
						+ propertyCode + "' ";
			...
			if (reportType.equals("Summary")) {
				subDataset = (JRDesignDataset) datasetMap
						.get("dsSalesOrderListSummary");
			} else {
				subDataset = (JRDesignDataset) datasetMap
						.get("dsSalesOrderListDetail");
			}
			subDataset.setQuery(subQuery);
			JasperReport jr = JasperCompileManager.compileReport(jd);
			HashMap hm = new HashMap();
	...
	@SuppressWarnings("rawtypes")
	private boolean funPrepardBOMData(clsSalesOrderHdModel SOModel,
			clsSalesOrderDtl obSODtl, String clientCode, String userCode) {
		...
		objSOBOMService.funDeleteSalesOrderBom(strSOCode,
				obSODtl.getStrProdCode(), clientCode);
		funGetChild(soQty, soWt, obSODtl.getStrProdCode(), clientCode);

		// itearate map and its inner class form saveing
		for (Map.Entry<String, ArrayList<clsInnerSalesBom>> entry : mapSOBOMcls
				.entrySet()) {
			// System.out.println(entry.getKey());
			/* from getting process Code */
			List listProdObject = objProductMasterService
					.funGetProdProcessList(obSODtl.getStrProdCode(), clientCode);
			...
			if (mapSOBOMcls.containsKey(entry.getKey())) {
				...
				for (clsInnerSalesBom objInn : listSOBOM) {
					/* from getting process Code */
					List listParentProdObject = objProductMasterService
							.funGetProdProcessList(entry.getKey(), clientCode);
					...
					if (listParentProdObject.size() != 0) {
						...
						strParentProdProcess = objParentProdProcess
								.getStrProcessCode();
					}
					// ///////////////////////////////////
					...
					SOBOMModel.setDteDateCreated(objGlobal
							.funGetCurrentDateTime("yyyy-MM-dd"));
					SOBOMModel.setDteLastModified(objGlobal
							.funGetCurrentDateTime("yyyy-MM-dd"));
		...
		if (mapSOBOMcls.isEmpty()) {
			...
			SOBOMModel.setDblWeight(obSODtl.getDblQty()
					* obSODtl.getDblWeight());
			SOBOMModel.setIntindex(new Long(i));
			SOBOMModel.setStrRemarks(obSODtl.getStrRemarks());
			SOBOMModel.setStrUserCreated(userCode);
			SOBOMModel.setStrUserModified(userCode);
	...
	private int funGetChild(double soQty, double soWt, String ProdCode,
			String clientCode) {
		int retVal = 0;
		List listBomCode = objRecipeMasterService.funGetBOMCode(ProdCode,
				clientCode);
		if (listBomCode.size() != 0) {
			clsBomHdModel objBomModel = (clsBomHdModel) listBomCode.get(0);

			List listBOMDtl = objRecipeMasterService.funGetBOMDtl(clientCode,
					objBomModel.getStrBOMCode());
			...
			for (int i = 0; i < listBOMDtl.size(); i++) {
				...
				funGetChild(childProdQty, childProdWeight, childProdCode,
						clientCode);
	...
	public ModelAndView funOpenSalesChar(Map<String, Object> model,
			HttpServletRequest req) {
		String urlHits = "1";
		...
		@SuppressWarnings("rawtypes")
		List listProdChar = objGlobalFunctionsService
				.funGetList("select a.strCharCode,b.strCharName from tblprodchar a,tblcharacteristics b where a.strCharCode=b.strCharCode and a.strProdCode='"
						+ prodCode
						+ "' and a.strClientCode='"
						+ clientCode
						+ "'");
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmTransectionProdChar", "command",
					new clsTransectionProdCharBean());
		} else if ("1".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmTransectionProdChar", "command",
					new clsTransectionProdCharBean());
		} else {
	...
	public double funGetCustAvg(String custCode, String prodCode,
			String dteSODate, HttpServletRequest req) {
		...
		String sqlAvgCustProd = " select sum(b.dblQty)/4 from tblsalesorderhd a ,tblsalesorderdtl b  "
				+ " where  a.strSOCode=b.strSOCode " + "  and a.strCustCode='"
				+ custCode
				+ "' "
				+ "  and b.strProdCode='"
				+ prodCode
	...
	@RequestMapping(value = "/SalesOrderCustAvg", method = RequestMethod.GET)
	public @ResponseBody String funGetCustAvgAddRow(
			@RequestParam("custCode") String custCode,
			@RequestParam("prodCode") String prodCode,
			@RequestParam("dteSODate") String dteSODate, HttpServletRequest req) {
		String retVal = "NV";
		double retAvgValue = 0.00;
		String clientCode = req.getSession().getAttribute("clientCode")
				.toString();
		...
		if (objSetup.getStrShowAvgQtyInSO().equals("Y")) {
			...
			String sqlAvgCustProd = " select sum(b.dblQty)/4 from tblsalesorderhd a ,tblsalesorderdtl b  "
					...
					+ "' "
					+ " and b.strClientCode='" + clientCode + "' ; ";
			List list = objGlobalFunctionsService.funGetList(sqlAvgCustProd,
					"sql");
	...
	public ModelAndView funOpenPOforMR(Map<String, Object> model,
			HttpServletRequest req) {
		return new ModelAndView("frmLoadMultiCustomer", "command",
				new clsReportBean());
	}
	...
	public @ResponseBody List funLoadCustomerDetail(HttpServletRequest request) {
		...
		String propCode = request.getSession().getAttribute("propertyCode")
				.toString();
		List custHelpList = objPartyMasterService.funGetListCustomer(
				clientCode, propCode);
		return custHelpList;
	}
	...
	public @ResponseBody List funPopulateAllProductDataOfAllCustomerl(
			@RequestParam("custCodes") String custCodes,
			@RequestParam("stdOrder") String strStdOrder,
			@RequestParam("dteSODate") String dteSODate,
			@RequestParam("locCode") String locCode, HttpServletRequest req) {
		...
		List<clsProductMasterModel> prodList = objProductMasterService
				.funGetALLProducedlProduct(clientCode);
										...
										// consolidate Standard vaule of qty
		{
			...
			String sqlSTDProdQty = " select a.strProdCode,sum(a.dblStandardValue) from tblprodsuppmaster a where a.strSuppCode IN  "
					+ strCustCode
					+ "  "
					+ " and a.strClientCode='"
					+ clientCode + "' " + " group BY a.strProdCode ";
			List listStd = objGlobalFunctionsService.funGetList(sqlSTDProdQty,
					"sql");
				...
				for (int i = 0; i < listStd.size(); i++) {
					...
					while (litr.hasNext()) {
						...
						if (objLtIT.getStrProdCode().equals(pCode)) {
							...
							if (objSetup.getStrShowStockInSO() != null
									&& objSetup.getStrShowStockInSO().equals(
											"Y")) {
								String proprtyWiseStock = "N";
								double dblStk = objGlobal
										.funGetCurrentStockForProduct(pCode,
												locCode, clientCode, userCode,
												startDate, dteSODate,
												proprtyWiseStock);
				...
				// if you are not select any customer then show all product
		{
			for (String cCode : tempstrCustCodes) {
				...
				while (litr.hasNext()) {
					...
					double avgValue = Double.parseDouble(funGetCustAvgAddRow(
							cCode, pCode, dteSODate, req));
					avgValue = Math.round(avgValue);
					Double dAvg = new Double(avgValue);
					int intValue = dAvg.intValue();
					avgValue = (double) intValue;
					objLtIT.setDblAcceptQty(objLtIT.getDblAcceptQty()
							+ avgValue);
					if (objSetup.getStrShowStockInSO() != null
							&& objSetup.getStrShowStockInSO().equals("Y")) {
						double dblStk = objGlobal.funGetCurrentStockForProduct(
								pCode, locCode, clientCode, userCode,
								startDate, dteSODate, propertyCode);
						objLtIT.setDblStock(dblStk);
					}
	...
	public @ResponseBody String funGetProdStk(
			@RequestParam("prodCode") String prodCode,
			@RequestParam("locCode") String locCode, HttpServletRequest req) {
	...
	public @ResponseBody List funLoadAutoSalesOrderForPI(
			@RequestParam("soCode") String soCode, HttpServletRequest req) {
		String clientCode = req.getSession().getAttribute("clientCode")
				.toString();
		String userCode = req.getSession().getAttribute("usercode").toString();
			...
			for (Object obj : listModel) {
				...
				for (Object objSO : objSalesDtlModelList) {
					...
					if (soDtl.getStrProdCode().equals(soBom.getStrParentCode())) {
						objPI.setDblQty(soBom.getDblQty()
								* soDtl.getDblAcceptQty());
	...
	public @ResponseBody List<clsSalesOrderTaxDtlModel> funLoadGRNDtl(
			Map<String, Object> model,
			@RequestParam("strSOCode") String strSOCode,
			HttpServletRequest request) {
		objGlobal = new clsGlobalFunctions();
		String clientCode = request.getSession().getAttribute("clientCode")
				.toString();
		...
		String sql = "select strTaxCode,strTaxDesc,strTaxableAmt,strTaxAmt from clsSalesOrderTaxDtlModel "
				+ "where strSOCode='"
				+ strSOCode
				+ "' and strClientCode='"
				+ clientCode + "'";
	...
	private JasperPrint funCallSalesOrder40Report(String GCode, String SOCode,
			String type, HttpServletResponse resp, HttpServletRequest req) {
		...
		try {
			...
			String reportName = servletContext
					.getRealPath("/WEB-INF/reports/webcrm/rptSalesOrderKOT.jrxml");
			String imagePath = servletContext
					.getRealPath("/resources/images/company_Logo.png");
			...
			List listHeader = objGlobalFunctionsService.funGetList(sqlHeader,
					"sql");
			...
			String sql = "SELECT  c.strGName,b.strProdName, d.strSGName,sum(a.dblQty) as dblQty,a.strProdCode "
					+ " FROM tblsalesorderdtl a "
					+ " left outer join tblproductmaster b on a.strProdCode=b.strProdCode "
					+ " left outer join tblsubgroupmaster d on  b.strSGCode=d.strSGCode "
					+ " left  outer join  tblgroupmaster c on c.strGCode=d.strGCode "
					+ " WHERE a.strSOCode='"
					+ SOCode
					+ "'  and  c.strGCode='"
					+ GCode
					...
					+ " "
					+ "group by c.strGCode,a.strProdCode\r\n"
	...
	public void funPrintSalesOrderKOTOnSalesOrder(
			@RequestParam("rptSOCode") String salesOrderCode,
			HttpServletResponse resp, HttpServletRequest req) {
	...
	public void funOpen40Report(
		...
		try {
			...
			String sqlHd = " SELECT c.strGCode, c.strGName,b.strProdName, d.strSGName FROM tblsalesorderdtl a, tblproductmaster b, tblsubgroupmaster d, tblgroupmaster c "
					+ " WHERE a.strSOCode='"
					+ SOCode
					+ "' and  c.strGCode=d.strGCode "
					+ " and b.strSGCode=d.strSGCode and a.strProdCode=b.strProdCode "
					+ " AND a.strClientCode = b.strClientCode AND b.strClientCode = c.strClientCode AND c.strClientCode = d.strClientCode "
					+ " group by c.strGCode;\r\n" + "";
			List list = objGlobalFunctionsService.funGetList(sqlHd, "sql");
			...
			for (int ii = 0; ii < list.size(); ii++) {
				...
				if (!list.isEmpty()) {
					...
					JasperPrint jp = funCallSalesOrder40Report(groupCode,
							SOCode, type, resp, req);
					 ...
					 * printRequestAttributeSet = new
					 * HashPrintRequestAttributeSet();
					 * printRequestAttributeSet.add(MediaSizeName.NA_LETTER);
					 * printRequestAttributeSet.add(new Copies(1));
					 * 
					 * // these are deprecated exporter = new
					 * JRPrintServiceExporter();
					 * exporter.setParameter(JRExporterParameter.JASPER_PRINT,
					 * jp);
					 * exporter.setParameter(JRPrintServiceExporterParameter
					 * .PRINT_SERVICE, printService[selectedService]);
					 * exporter.setParameter(JRPrintServiceExporterParameter.
					 ...
					 * Boolean.FALSE); exporter.exportReport(); // // DocFlavor
					 * flavor = DocFlavor.INPUT_STREAM.AUTOSENSE; // DocPrintJob
					 * job = printService[0].createPrintJob(); //
					 * FileInputStream fis = new FileInputStream(filename); //
					 * DocAttributeSet das = new HashDocAttributeSet(); // Doc
					 * doc = new SimpleDoc(fis, flavor, das); // job.print(doc,
					 * printerReqAtt);
					 * 
					 * } catch (Exception e) {
					 * 
					...
					/*
					 * PrinterJob job = PrinterJob.getPrinterJob(); Create an
					 * array of PrintServices PrintService[] services =
					 * PrintServiceLookup.lookupPrintServices(null, null); int
					 * selectedService = 0; Scan found services to see if anyone
					 * suits our needs for (int i = 0; i < services.length; i++)
					 * { if
					 * (services[i].getName().toUpperCase().contains("POS80B"))
					 * { selectedService = i; } }
					 * job.setPrintService(services[selectedService]);
					 * PrintRequestAttributeSet printRequestAttributeSet = new
					 * HashPrintRequestAttributeSet(); MediaSizeName
					 * mediaSizeName = MediaSize.findMedia(4, 4,
					 * MediaPrintableArea.INCH);
					 * printRequestAttributeSet.add(mediaSizeName);
					 * printRequestAttributeSet.add(new Copies(1));
					 * JRPrintServiceExporter printerExporter; printerExporter =
					 * new JRPrintServiceExporter();
					 * printerExporter.setParameter
					 * (JRExporterParameter.JASPER_PRINT, jp); We set the
					 * selected service and pass it as a paramenter
					 * printerExporter
					 ...
					 * printerExporter.setParameter
					 * (JRPrintServiceExporterParameter. DISPLAY_PRINT_DIALOG,
					 * Boolean.TRUE); printerExporter.exportReport();
					 */
					...
					printerExporter.setParameter(
							JRExporterParameter.JASPER_PRINT, jp);
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public void funSaveAudit(String soCode, String strTransMode,
			HttpServletRequest req) {
		try {
			String clientCode = req.getSession().getAttribute("clientCode")
					.toString();
			String userCode = req.getSession().getAttribute("usercode")
					.toString();
			clsSalesOrderHdModel soModel = objSalesOrderService
					.funGetSalesOrderHd(soCode, clientCode);
			if (null != soModel) {
				List<Object> objSalesDtlModelList = objSalesOrderService
						.funGetSalesOrderDtl(soCode, clientCode);
				if (null != objSalesDtlModelList
						&& objSalesDtlModelList.size() > 0) {
					String sql = "select count(*)+1 from tblaudithd where left(strTransCode,12)='"
							+ soModel.getStrSOCode() + "'";
					List list = objGlobalFunctionsService
							.funGetList(sql, "sql");
					if (!list.isEmpty()) {
						...
						for (int i = 0; i < objSalesDtlModelList.size(); i++) {
							...
							AuditMode
									.setDblUnitPrice(saleDtl.getDblUnitPrice());
							AuditMode.setDblTax(saleDtl.getDblTax());
							AuditMode.setDblTaxableAmt(saleDtl
									.getDblTaxableAmt());
							AuditMode.setDblTaxAmt(saleDtl.getDblTaxAmt());
							AuditMode.setStrTaxType(saleDtl.getStrTaxType());
							AuditMode.setDblTotalPrice(saleDtl
									.getDblTotalPrice());
							AuditMode.setStrClientCode(saleDtl
									.getStrClientCode());
