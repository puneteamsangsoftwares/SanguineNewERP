			...
			for (Map.Entry<String, List<clsInvoiceModelDtl>> entrySetInvoiceModelDtl : hmInvCustDtl.entrySet())
			{
				...
				if (objBean.getStrInvCode().isEmpty()) // New Entry
				{
					...
					String invCode ="";
						...
						*/
						String sql = "select ifnull(max(MID(a.strInvCode,8,5)),'' ) " + " from tblinvoicehd a where MID(a.strInvCode,5,1) = '" + transYear + "' " + ""
							 	  + "  and MID(a.strInvCode,1,2) = '" + propCode + "' and strClientCode='" + clientCode + "' ";
						String sqlAudit = " select ifnull(max(MID(a.strTransCode,8,5)),'' ) " + ""
