...
public class clsExportTallyFileController {
	...
	private void funPurchaseXMLFileFormate1(clsReportBean objBean, String clientCode, HttpServletRequest req) {
		...
		try {
			...
			String sql = " select a.strGRNCode,a.strSuppCode,c.strPName,a.strAgainst,a.strBillNo,Date(a.dtBillDate) , " + " a.dblTotal,ifnull(d.strTallyCode,''),a.strUserModified ,DATE_FORMAT(a.dtDueDate,'%d-%b-%Y') ,DATE_FORMAT(a.dtGRNDate,'%b')  ,e.strLocName ,a.dblExtra" + " from tblgrnhd a " + " left outer join tblgrntaxdtl b on a.strGRNCode=b.strGRNCode  and b.strClientCode='" + clientCode + "' "
					+ " left outer join tblpartymaster c on a.strSuppCode=c.strPCode and c.strClientCode='" + clientCode + "' " + " left outer join tbltallylinkup d on d.strGroupCode=a.strSuppCode " + ",tbllocationmaster e  where  date(a.dtGRNDate) between  '" + objBean.getDteFromDate() + "' and '" + objBean.getDteToDate() + "'  " + " and a.strClientCode='" + clientCode + "' and a.dblTotal >0 and a.strLocCode=e.strLocCode and a.strGRNCode like '" + strPropCode
					+ "'  ";
			...
			for (int cnt = 0; cnt < list.size(); cnt++) {
				...
				sql="select b.strProdCode,b.strProdName from tblgrndtl a ,tblproductmaster b"
				  + " where a.strGRNCode='"+grnNo+"' and a.strProdCode=b.strProdCode ";
				...
				for (int Pcnt = 0; Pcnt < listOFAllProd.size(); Pcnt++) {
				    ...
				    if(strAllProd.isEmpty())
				    {
				    	strAllProd=arrPObj[1].toString();
				    }
				    else
				    {
				    	strAllProd +=","+arrPObj[1].toString();
				...
				tallyMessageElement.appendChild(getDataFormate1(doc, String.valueOf(vNo), grnDate, strBillNo, "No", String.valueOf(vNo), grnNo, pNameAlise, "No", dblTotal, objBean, clientCode, companyName, objBean.getStrUserCode(), billDateddMMyyyy, billMonth,strAllProd, arrObj[11].toString(),Double.parseDouble(arrObj[12].toString())));
	...
	//	voucher.appendChild(getVoucherElements(doc, voucher, "GUID", guid));
		voucher.appendChild(getVoucherElements(doc, voucher, "VOUCHERTYPENAME"," Purchase"));
	...
	//	voucher.appendChild(getVoucherElements(doc, voucher, "PARTYLEDGERNAME", strSuppName));
		...
		voucher.appendChild(getVoucherElements(doc, voucher, "ISCOSTCENTRE","YES"));
