...
public class clsPMSPaymentServiceImpl implements clsPMSPaymentService {
	...
	public clsPMSPaymentHdModel funPreparePaymentModel(clsPMSPaymentBean objPaymentBean, String clientCode, HttpServletRequest request, String userCode) {
		...
		if (objPaymentBean.getStrReceiptNo()!=null && objPaymentBean.getStrReceiptNo().isEmpty()) {
			...
			objModel=objPaymentDao.funGetPaymentModel(objPaymentBean.getStrReceiptNo(),clientCode);
			objModel.setDblPaidAmt(objPaymentBean.getDblReceiptAmt());
			objModel.setDblReceiptAmt(objPaymentBean.getDblReceiptAmt());
			objModel.setDteDateEdited(objGlobal.funGetCurrentDateTime("yyyy-MM-dd"));
			objModel.setStrUserEdited(userCode);
			
			List<clsPMSPaymentReceiptDtl> listOldPaymentReceiptDtlModel =objModel.getListPaymentRecieptDtlModel();
			clsPMSPaymentReceiptDtl objPaymentOldReceiptDtlModel=listOldPaymentReceiptDtlModel.get(0); 
			objPaymentOldReceiptDtlModel.setStrSettlementCode(objPaymentBean.getStrSettlementCode());
			objPaymentOldReceiptDtlModel.setStrCardNo(objPaymentBean.getStrCardNo());			
			objPaymentOldReceiptDtlModel.setDblSettlementAmt(objPaymentBean.getDblReceiptAmt());
			objPaymentOldReceiptDtlModel.setStrRemarks(objPaymentBean.getStrRemarks());
			objModel.setListPaymentRecieptDtlModel(listOldPaymentReceiptDtlModel);
