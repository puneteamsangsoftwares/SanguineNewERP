...
//				String noOfRoom = objPropertySetupModel.getStrRoomLimit();.
				...
				String sqlPayDtlAgainstRes = "SELECT DATE_FORMAT(DATE(a.dteReceiptDate),'%d-%m-%Y'),a.strReceiptNo,CONCAT(c.strSettlementDesc,' - ',a.strAgainst),'0.00' AS debitAmt,"
				...
				if(paymentDtlListAgainstRes!=null && paymentDtlListAgainstRes.size()>0){
				 ...
				 sqlPayDtlAgainstRes = "SELECT DATE_FORMAT(DATE(a.dteReceiptDate),'%d-%m-%Y'),a.strReceiptNo,CONCAT(c.strSettlementDesc,' - ',a.strAgainst),'0.00' AS debitAmt,"
						+ " b.dblSettlementAmt AS creditAmt,'0.00' AS balance"
						+ " FROM tblreceipthd a"
						+ " LEFT OUTER "
						+ " JOIN tblreceiptdtl b ON a.strReceiptNo=b.strReceiptNo "
			...
			String sqlPaymentDtl = "SELECT DATE_FORMAT(date(c.dteReceiptDate),'%d-%m-%Y'),c.strReceiptNo,CONCAT(e.strSettlementDesc,' - ',c.strAgainst),'0.00' as debitAmt,d.dblSettlementAmt as creditAmt" + " ,'0.00' as "
					+ " balance " + " FROM tblfoliohd a LEFT OUTER JOIN tblfoliodtl b ON a.strFolioNo=b.strFolioNo AND a.strClientCode='"+clientCode+"' "
	...
	// Save folio posting
		@RequestMapping(value = "/rptFolioFostingPrinting", method = RequestMethod.GET)
		public void funGenerateFolioFostingPrintingReport(@RequestParam("folioNo") String folioNo, @RequestParam("folioDocNo") String folioDocNo,HttpServletRequest req, HttpServletResponse resp)
		{
...
//					String noOfRoom = objPropertySetupModel.getStrRoomLimit();.
					...
					String sqlFolioDtl = "SELECT DATE_FORMAT(b.dteDocDate,'%d-%m-%Y'),b.strDocNo, CONCAT(IFNULL(SUBSTRING_INDEX(SUBSTRING_INDEX(b.strPerticulars,'(', -1),')',1),''),' ',b.strRemark),"
							...
							+ " AND b.strClientCode='"+clientCode+"'" + ""
							+ " LEFT OUTER JOIN tblroom d  on  b.strRevenueCode=d.strRoomCode "
							+ " WHERE a.strFolioNo='" + folioNo + "' and b.strRevenueType!='Discount' and b.dblDebitAmt > 0 and b.strDocNo='"+folioDocNo+"' "
							+ " order by b.dteDocDate ASC";
