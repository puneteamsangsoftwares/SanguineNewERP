...
@Controller
public class clsDebtorAgeingReportController {
	...
	@RequestMapping(value = "/frmDebtorAgeingReport", method = RequestMethod.GET)
	...
	@RequestMapping(value = "/getDebtorAgeingData", method = RequestMethod.GET)
	...
	@RequestMapping(value = "/exportDebtorAgeingData", method = RequestMethod.GET)
	private ModelAndView funExportDebtorAgeingData(HttpServletRequest request,HttpServletResponse response) 
	{
	...
	private Map<String,List<Object>> funGetAgeingMapData(HttpServletRequest request)
	{
		...
		try {
			...
			String fromDateToCalculateOutStanding = intFromDateYearForOutStanding + "-" + intFromDateMonthForOutStanding + "-"+ intFromDateDayForOutStanding;
			...
			String toDateToCalculateuteOutStanding = intToDateYearForOutStanding + "-" + intToDateMonthForOutStanding + "-"+ intToDateDayForOutStanding;
			 ...
			 */
			Map<String,clsCreditorOutStandingReportBean> hmOutstanding = objGlobalFunctions.funCalCreditorDebtorOPBalance(glCode, clientCode, fromDateToCalculateBal, toDateToCalculateBal, DCType, request);
			...
			sbSql.append("select debtor.strDebtorCode,'',debtor.strCrDr, sum(ifnull(debtor.dblAmt,0.00)) "
				...
				+ " where debtor.strAccountCode='"+glCode+"' and debtor.strCrDr='Cr' and date(hd.dteVouchDate) between '" + fromDateToCalculateOutStanding + "' AND '" + toDateToCalculateuteOutStanding + "' "
			...
			if (listJVAmt != null && listJVAmt.size() > 0) {
				for (int j = 0; j < listJVAmt.size(); j++) {
					...
					objOutStBean.setDblBalAmt(objOutStBean.getDblBalAmt()-objOutStBean.getDblCrAmt());
			...
			String toDateForAgeing = intToDateYearForAgeing + "-" + intToDateMonthForAgeing + "-"+ intToDateDayForAgeing;
			...
			String fromDateForAgeing=intFromDateYearForAgeing + "-" + intFromDateMonthForAgeing + "-"+ intFromDateDayForAgeing;;
			...
			while(true)
			{
				...
				sbSql.append("SELECT dm.strDebtorCode,dm.strDebtorFullName,ifnull(c.CrDr,'Dr'), ifnull(SUM(c.DebirAmt),0)  ");
				sbSql.append("FROM tblsundarydebtormaster dm "
						   + "left Outer join   ");
				sbSql.append("(  ");
				sbSql.append("SELECT debtor.strDebtorCode DebtorCode,' ' blank,debtor.strCrDr CrDr, SUM(IFNULL(debtor.dblAmt,0.00)) DebirAmt   ");
				...
				sbSql.append("WHERE debtor.strAccountCode='" + glCode
						+ "' AND debtor.strCrDr='Dr' AND DATE(hd.dteVouchDate) BETWEEN '" + fromDateForAgeing
				...
				//sbSql.append("and debtor.strDebtorCode='D00000355'   ");
				sbSql.append("GROUP BY debtor.strCrDr,debtor.strDebtorCode    ");
				sbSql.append(") c  on dm.strDebtorCode= c.DebtorCode ");				
				sbSql.append("GROUP BY dm.strDebtorCode  ");						
				...
				if (listDrJVAmt != null && listDrJVAmt.size() > 0) 
				{
					for (int j = 0; j < listDrJVAmt.size(); j++) 
					{
						...
						double debitAmt=Double.parseDouble(arrObj[3].toString());
						...
						{																			
							...
							if(debitAmt < oldOutStandingAmt )
							{
								newOutStandingAmt=oldOutStandingAmt-debitAmt;
							}
							else// oldOutStandingAmt < debitAmt 
							{
								debitAmt=oldOutStandingAmt;
							}
				...
				fromDateForAgeing=intFromDateYearForAgeing + "-" + intFromDateMonthForAgeing + "-"+ intFromDateDayForAgeing;;
