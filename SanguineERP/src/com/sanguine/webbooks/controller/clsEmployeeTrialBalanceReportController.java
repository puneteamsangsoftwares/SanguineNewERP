...
@Controller
public class clsEmployeeTrialBalanceReportController {
	...
	@Autowired
	clsTrialBalanceReportController objTrialBalanceReportController;
	
	@RequestMapping(value = "/frmEmployeeTrailBalanceReport", method = RequestMethod.GET)
	...
	@RequestMapping(value = "/rptEmployeeTrialBalanceReport", method = RequestMethod.GET)
	private void funReport(@ModelAttribute("command") clsCreditorOutStandingReportBean objBean, HttpServletResponse resp, HttpServletRequest req) {
		try {
			...
			String reportName = servletContext.getRealPath("/WEB-INF/reports/webbooks/rptEmployeeTrialBalanceReport.jrxml");
			 ...
			 List listDebtor = objGlobalFunctionsService.funGetListModuleWise(sbSqllink.toString(), "sql");
					...
					for (int i = 0; i < listDebtor.size(); i++) {
						...
						try {
							...
							funProcessWebBookEmployeeTrailBal(sunCrAccountCode, sunCrCode, startDate, newToDate, clientCode, userCode, propertyCode, req, resp);

						
							sql = " select ifnull(sum(bal),0) from  (select (IF(a.strCrDr='Dr',a.dblOpeningbal,0) - IF(a.strCrDr='Cr',a.dblOpeningbal,0)) bal " 
								+ " from tblemployeeopeningbalance a " + " where a.strEmployeeCode='" + sunCrCode + "' and a.strAccountCode='" + sunCrAccountCode + "' AND a.strClientCode='" + clientCode + "' " + " UNION All "
							...
							List listOpBalOld = objGlobalFunctionsService.funGetListModuleWise(sql, "sql");
								...
								funProcessWebBookEmployeeTrailBal(sunCrAccountCode, sunCrCode, dteFromDate, dteToDate, clientCode, userCode, propertyCode, req, resp);
	...
	public int funProcessWebBookEmployeeTrailBal(String acCode, String detorCretditorCode, String fromDate, String toDate, String clientCode, String userCode, String propertyCode, HttpServletRequest req, HttpServletResponse resp) {
		
		objGlobal.funDeleteAndInsertWebBookLedgerTable(clientCode, userCode, propertyCode);
		// jv
