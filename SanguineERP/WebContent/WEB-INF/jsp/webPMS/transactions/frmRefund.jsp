...
<script type="text/javascript">
	...
	function funSetData(code){
		...
		switch(fieldName){
			...
			case 'BillForRefund':
				funSetPaymentGuestDataAgainstBill(code,"Bill");
				break;
	...
	function funSetReceiptData(code){
		...
		$.ajax({
			type : "GET",
			url : getContextPath()+ "/loadRefundData.html?receiptNo=" + code,
			dataType : "json",
			success : function(response){ 
	...
	function funSetPaymentGuestDataAgainstBill(code,type)
	{
		var guest=$("#txtCreditName").val();
		var searchUrl=getContextPath()+"/loadPaymentGuestDetailsForRefund.html?docCode="+code+ "&docName=" + type;
		//var searchUrl=getContextPath()+"/loadPaymentGuestDetailsForRefund.html?docCode="+code+ "&docName=" + type+"&guestCode"= +guest;
		$.ajax({
	...
	function funHelp1(transactionName)
	{
		...
		var guest=$("#txtCreditName").val();
		fieldName=transactionName;
		if(fieldName=='BillForRefund')
		{
			window.open("searchform.html?formname="+transactionName+"&guestCode="+guest+"&searchText=","mywindow","directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=no,width=900,height=600,left=400px");
		}
		else
		{
			window.open("searchform.html?formname="+transactionName+"&searchText=","mywindow","directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=no,width=900,height=600,left=400px");
				...
				if(isOk)
 				{
	 				...
	 				checkAgainst='Bill';
	    			window.open(getContextPath()+"/rptRefundRecipt.html?reciptNo="+reciptNo+"&checkAgainst="+checkAgainst,'_blank');
...
<body>
  <div class="container masterTable">
	<label id="formHeading">Refund</label>
	  <s:form name="Payment" method="GET" action="saveRefund.html">
	
	   <div class="row">
            <div class="col-md-2"><label>Refund Receipt No</label>
		            <s:input id="txtReceiptNo" path="strReceiptNo" readonly="readonly" ondblclick="funHelp('refundReceiptNo');" style="height: 45%;" class="searchTextBox"></s:input>
			</div>
		
	        <div class="col-md-4">
	   	       <div class="row"><div class="col-md-6" id="trGuest"><label>Guest Name</label>
