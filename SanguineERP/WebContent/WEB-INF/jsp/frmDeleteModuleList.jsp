...
var members,propertyName,locationName;
	...
	function pageselectCallback(page_index, jq){
	    ...
	    var newcontent = '<table id="tblFromDtl" class="transTablex col2-center" style="width: 100%;font-size:11px;font-weight: bold;"><tr style="background-color:#c0c0c0;"><td>Module Name</td><td>Select<input type="checkbox" id="chkALL" onclick="funSelectAll();"/></td></tr>';
...
// 			document.all["cmbLoction"].style.display = 'block';
	...
	{
		searchUrl = getContextPath()+ "/loadPropertyName.html?propName="+property;
		//alert(searchUrl);
		$.ajax({
			type : "GET",
			url : searchUrl,
			...
			success : function(response) {
				...
				$.each(response, function(i, items) 
						{ 
						  $('#cmbProperty').append( $('<option></option>').val(items).html(items) );
						});
				var propName=$('#cmbProperty').find('option:selected').text();
				//alert(propName);
				propertyName=propName;
				funLoctionName(propName);
	...
	{		
		...
		$.ajax({
			...
			success : function(response) {				
				...
				locationName=$('#cmbLoction').find('option:selected').text();
				//alert(response);
...
<%-- 				<td><select id="cmbLoction"  path="strLocName" style="display:none" class="BoxW124px"></select></td> --%>
		...
		<div id="Pagination" class="pagination"></div>
			...
			<br>
			<p align="center">
			    <input type="button" value="Submit" class="btn btn-primary center-block" class="form_button" style="height:27px;" onclick="funSubmit_click();" />
			    &nbsp;
			    <input type="reset" value="Reset" class="btn btn-primary center-block" class="form_button" style="height:27px;" onclick="funResetFields();" /><br/>
			</p>
		</form>
	</div>
</body>
